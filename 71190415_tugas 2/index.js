...
const PetCardComponent = (pet) => {
  ...
  const breed = pet.breeds[0] ? pet.breeds[0].name : 'Unknown Breed';
  const category = pet.categories[0] ? pet.categories[0].name : 'Unknown Category';
  ...
  return `<div class="card my-3 mx-2" style="width: 20%">
    ...
    <div class="card-body">
      ...
      <p>${pet.description}</p>
      <span class="badge badge-pill badge-info">${pet.id}</span>
      <span class="badge badge-pill badge-warning">Weight: ${pet.weight.metric}</span>
      <span class="badge badge-pill badge-danger">Height: ${pet.height.metric}</span>
    </div>
  </div>`;
};
...
const searchPetByKey = (key) => {
  ...
  return petList.filter((pet) => {
    return pet.breeds[0].name.toLowerCase().includes(key.toLowerCase()) ||
      pet.categories[0].name.toLowerCase().includes(key.toLowerCase());
  });
};
